...
function smashed(stl) {
    document.getElementById("desc").innerHTML = "Smashed: " + smsh.length;
...
function go() {
    document.getElementById("thebutton").innerHTML = "Workin' on it";
    document.getElementById("desc").innerHTML = "Jailbreaking the device...";
    dgc();
    setTimeout(go_, 400);
}
...
window.onload = function() {
    ...
    if (!ver) {
	document.getElementById("w").innerHTML = "<p id=\"error\">This device is not running iOS.</p>";
    } else if (ver.major != 9 || ver.minor != 3 || (ver.patch >= 4)) {
	document.getElementById("w").innerHTML = `<p id=\"error\">This supports only iOS 9.3.0 ~ 9.3.3. iOS ${ver.major}.${ver.minor}.${ver.patch} is not supported!</p>`;
    }
}
